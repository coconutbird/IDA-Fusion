...
if (WIN32)
    # Determine the correct library directory name based on architecture
    if (CMAKE_SIZEOF_VOID_P EQUAL 8)
        set(IDA_LIB_DIR_NAME "x64_win_vc_64")
    else ()
        set(IDA_LIB_DIR_NAME "x64_win_vc_32")
    endif ()

    # Try src/lib first (GitHub SDK structure)
    if (EXISTS "${IDA_SDK_DIR}/src/lib/${IDA_LIB_DIR_NAME}")
        set(IDASDK_LIBRARY_DIRS "${IDA_SDK_DIR}/src/lib/${IDA_LIB_DIR_NAME}")
    elseif (EXISTS "${IDA_SDK_DIR}/src/lib")
        set(IDASDK_LIBRARY_DIRS "${IDA_SDK_DIR}/src/lib")
        # Try lib directory (standard SDK structure)
    elseif (EXISTS "${IDA_SDK_DIR}/lib/${IDA_LIB_DIR_NAME}")
        set(IDASDK_LIBRARY_DIRS "${IDA_SDK_DIR}/lib/${IDA_LIB_DIR_NAME}")
    elseif (EXISTS "${IDA_SDK_DIR}/lib")
        set(IDASDK_LIBRARY_DIRS "${IDA_SDK_DIR}/lib")
    endif ()
elseif (UNIX AND NOT APPLE)
    if (CMAKE_SIZEOF_VOID_P EQUAL 8)
        set(IDA_LIB_DIR_NAME "x64_linux_gcc_64")
    else ()
        set(IDA_LIB_DIR_NAME "x64_linux_gcc_32")
    endif ()
...
elseif (APPLE)
    if (CMAKE_SIZEOF_VOID_P EQUAL 8)
        set(IDA_LIB_DIR_NAME "x64_mac_gcc_64")
    else ()
        set(IDA_LIB_DIR_NAME "x64_mac_gcc_32")
    endif ()
...
if (NOT IDASDK_LIBRARIES)
    message(ERROR "IDA library not found in ${IDASDK_LIBRARY_DIRS}. Linking may fail.")
endif ()
