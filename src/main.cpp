...
bool idaapi plugin_run(size_t arg) {
  std::string form_str = n_utils::format("IDA-Fusion for %.1f+\n"
  ...
  case 4: {
    ...
    if (!ask_form("Fusion — Enter CODE/IDA signature\n"
                  "<Signature:A5:8192:100>",
            &signature_to_find))
      break;

    n_signature::find(signature_to_find,
        {false,
            (bool) (n_settings::data & FLAG_STOP_AT_FIRST_SIGNATURE_FOUND),
            0,
            0,
            static_cast<bool>(n_settings::data & FLAG_AUTO_JUMP_TO_FOUND_SIGNATURES)});
    break;
  }
  case 5: {
...
EXTERN plugin_t PLUGIN = {IDP_INTERFACE_VERSION,
    PLUGIN_PROC,
    plugin_init,
    nullptr,
    plugin_run,
