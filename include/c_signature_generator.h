    ...
    if (style == SIGNATURE_STYLE_FNV1A)
      ...
      for (u32 i = 0; i < bytes.size(); i++) {
        ...
        if (style == SIGNATURE_STYLE_IDA) {
          if (i > 0) qsnprintf(sig + strlen(sig), sig_len_per_byte, " ");

          qsnprintf(sig + strlen(sig),
              sig_len_per_byte,
              imm[i] ? ((n_settings::data & FLAG_USE_DUAL_QUESTION_MARKS) ? "??" : "?") : "%02X",
              (u8) bytes[i]);
        } else if (style == SIGNATURE_STYLE_CODE)
          qsnprintf(sig + strlen(sig),
              sig_len_per_byte,
              imm[i] ? ((n_settings::data & FLAG_USE_UNICODE_WILDCARD) ? "\\x2A" : "\\x00")
                     : "\\x%02X",
              (u8) bytes[i]);
      }
