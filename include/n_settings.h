...
enum e_settings_flags {
  ...
  = 1 << 1, // When a region is selected in the current screen view, explicitly create a signature
            // for these bytes
  FLAG_SHOW_MNEMONIC_OPCODES_SIGGED
  = 1 << 2, // Shows the mnemonics for all of the bytes in the signature
  FLAG_COPY_CREATED_SIGNATURES_TO_CB
  = 1 << 3, // Copy any created signatures to the clipboard automatically
  FLAG_INCLUDE_MASK_FOR_CODE_SIGS = 1 << 4, // Include the mask for code signatures
  FLAG_ALLOW_SIG_CREATION_IN_DR
  = 1 << 5, // Allow signature creation in unknown, dangerous regions (assembly marked in red)
  FLAG_STOP_AT_FIRST_SIGNATURE_FOUND = 1 << 6, // Stop searching after one signature has been found
  FLAG_USE_DUAL_QUESTION_MARKS
  = 1 << 7, // Use "??" as wildcard instead of "?" for IDA style signatures
  FLAG_USE_UNICODE_WILDCARD
  = 1 << 8, // Use "2A" as wildcard instead of "00" for CODE style signatures
};
...
u32 data = FLAG_AUTO_JUMP_TO_FOUND_SIGNATURES | FLAG_COPY_SELECTED_BYTES_ONLY_IN_RANGE
           | FLAG_SHOW_MNEMONIC_OPCODES_SIGGED | FLAG_COPY_CREATED_SIGNATURES_TO_CB;

bool show_settings_dialog() {
  bool form_ok = ask_form("Fusion — Settings\n"
