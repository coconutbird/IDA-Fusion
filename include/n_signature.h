...
static std::vector<ea_t> find(std::string signature, s_signature_find_settings find_settings) {
  ...
  if (strstr(signature.c_str(), "\\x")) {
    ...
    if (!signature.empty() && signature[0] == ' ') signature.erase(0, 1);
  }
  ...
  while (true) {
    ...
    if (addr == find_settings.ignore_addr) continue;

    // Jump to the first address we find
    if (find_settings.jump_to_found_addr && ea.empty()) jumpto(addr);
    ...
    if (find_settings.stop_at_first) break;
  }
...
static void create(e_signature_style style) {
  if (!(n_settings::data & FLAG_ALLOW_SIG_CREATION_IN_DR)
      && get_func_num(get_screen_ea()) == 0xFFFFFFFF) {
    hide_wait_box();
    warning("[Fusion] `0x%llX` Is not in a valid assembly region.\n\nHint: You can disable this in "
            "the settings of Fusion.",
        get_screen_ea());
    return;
  }
  ...
  if ((n_settings::data & FLAG_COPY_SELECTED_BYTES_ONLY_IN_RANGE)
    ...
    for (ea_t addr = iterator.current(); true; addr = iterator.current()) {
      ...
      for (ea_t op_addr = addr; op_addr < (addr + insn.size); op_addr++)
        signature_generator.add(
            get_byte(op_addr), imm_offset > 0 && (op_addr - addr) >= imm_offset);
      ...
      if (!iterator.next_not_tail()) break;
    }
  } else {
    ...
    i8* mnemonic_opcodes = (n_settings::data & FLAG_SHOW_MNEMONIC_OPCODES_SIGGED)
                               ? (i8*) malloc(mnemonic_opcodes_len)
                               : nullptr;

    if (mnemonic_opcodes != nullptr) memset(mnemonic_opcodes, 0, mnemonic_opcodes_len);
    ...
    for (ea_t addr = iterator.current(); true; addr = iterator.current()) {
      ...
      if (n_settings::data & FLAG_SHOW_MNEMONIC_OPCODES_SIGGED) {
        qsnprintf(mnemonic_opcodes + strlen(mnemonic_opcodes),
            mnemonic_opcodes_len - strlen(mnemonic_opcodes),
            "+ %s\n",
            insn.get_canon_mnem(PH));
        replace_wait_box(
            "[Fusion] Creating signature for `0x%llX`\n\n%s", target_addr, mnemonic_opcodes);
      }
      ...
      // Attempt to search for this signature, if nothing is found then we have a unique signature
      {
        ...
        std::vector<ea_t> search_result
            = find(ida_sig, {true, true, target_addr, last_found_address, false});
        free(ida_sig);
    ...
    if (mnemonic_opcodes != nullptr) free(mnemonic_opcodes);
  }
